...
class Electra(nn.Module):
    def __init__(self, output_size=24005, device='cuda'):
        super().__init__()
        self.device = device
        ...
        self.output = nn.Linear(
            self.electra.config.hidden_size, output_size).to(device)

    def forward(self, sents, locs):
        sents = torch.tensor(sents).to(self.device)
        sents = self.electra(sents)[0]
        abbs = torch.stack([sents[n, idx, :]
                           for n, idx in enumerate(locs)])  # (B * M)
